...
func TestShellConfigPermissionsDenyWinsOverAllow(t *testing.T) {
	// Tool names follow this tree's naming (Bash/Read, not upstream's
	// bash/view); using the real names is what makes the final two
	// assertions actually exercise deny-wins rather than pass vacuously.
	store := loadCrushSh(t, `permissions allow Bash Read
permissions deny Bash`)
