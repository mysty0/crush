...
type BashParams struct {
	...
	AutoBackgroundAfter int    `json:"auto_background_after,omitempty" description:"Seconds to wait before automatically moving the command to a background job (default: 60). Keep this at or below 240: blocking a turn for longer lets the provider's prompt cache expire, which re-sends and re-caches the whole conversation on the next request."`
}
		...
		waitLoop:
			for {
				...
				case <-ctx.Done():
					...
					// Kill the shell and return error. Use a fresh context here
					// since ctx is already done — Kill's own internal timeout
					// still bounds how long this waits.
