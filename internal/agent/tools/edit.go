...
func commitFileChange(edit editContext, sessionID, filePath, oldContent, newContent string) error {
	...
	messageID := GetMessageFromContext(edit.ctx)
	file, err := edit.files.GetByPathAndSession(edit.ctx, filePath, sessionID)
	if err != nil {
		_, err = edit.files.Create(edit.ctx, sessionID, messageID, filePath, oldContent)
		if err != nil {
	...
	if file.Content != oldContent {
		...
		if _, err := edit.files.CreateVersion(edit.ctx, sessionID, messageID, filePath, oldContent); err != nil {
			slog.Error("Error creating file history version", "error", err)
		}
	}
	if _, err := edit.files.CreateVersion(edit.ctx, sessionID, messageID, filePath, newContent); err != nil {
		slog.Error("Error creating file history version", "error", err)
	}
