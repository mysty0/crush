...
2. Security Check: Default-restricted commands ({{ .BannedCommands }}) prompt the user for approval before running (and can be allowed for the session); if the user declines they return an error - explain to the user. Safe read-only commands execute without prompts
