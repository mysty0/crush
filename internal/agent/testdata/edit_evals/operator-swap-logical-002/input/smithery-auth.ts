...
export async function createSmitheryCliAuthSession(): Promise<SmitheryCliAuthSession> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to create Smithery auth session: ${response.status} ${response.statusText}`,
    );
  }
...
export async function getSmitheryApiKey(): Promise<string | undefined> {
  ...
  try {
    ...
    logger.warn('Failed to read Smithery auth file, treating as missing', {
      path: authPath,
      error,
    });
    return undefined;
...
export async function saveSmitheryApiKey(apiKey: string): Promise<void> {
  ...
  try {
    ...
    logger.warn('Could not set restrictive permissions on Smithery auth file', {
      path: authPath,
      error,
    });
  }
