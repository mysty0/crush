...
async function installPythonPackages(
  ...
  if (uvPath) {
    ...
    if (result.exitCode === 0) {
      return { success: true, usedManagedEnv: true };
    }
  }
  ...
  if (uvPath) {
    ...
    const installInManagedEnv =
      await $`${uvPath} pip install --python ${MANAGED_PYTHON_ENV} ${packages}`.nothrow();
...
async function handlePythonSetup(flags: { json?: boolean; check?: boolean }): Promise<void> {
  ...
  if (check.installedPackages.length > 0) {
    console.log(
      chalk.green(`${theme.status.success} Installed: ${check.installedPackages.join(', ')}`),
    );
  }
  ...
  if (!install.success) {
    ...
    if (install.usedManagedEnv) {
      if (check.uvPath) {
        ...
        console.error(
          chalk.dim(
            `  uv pip install --python ${MANAGED_PYTHON_ENV} ${check.missingPackages.join(' ')}`,
          ),
        );
      } else {
        console.error(chalk.dim(`  ${check.pythonPath} -m venv ${MANAGED_PYTHON_ENV}`));
        console.error(
          chalk.dim(`  ${managedPythonPath()} -m pip install ${check.missingPackages.join(' ')}`),
        );
      }
    } else {
      console.error(
        chalk.dim(
          `  ${check.uvPath ? 'uv pip' : 'pip'} install ${check.missingPackages.join(' ')}`,
        ),
      );
    }
    process.exit(1);
