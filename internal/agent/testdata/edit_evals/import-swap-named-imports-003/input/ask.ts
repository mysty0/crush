...
async function askSingleQuestion(
  ...
  const promptForCustomInput = async (): Promise<{ input: string | undefined }> => {
    ...
    const showCustomInput = () =>
      ui.editor('Enter your response:', undefined, dialogOptions, { promptStyle: true });
  ...
  async execute(
    ...
    const ui: UIContext = {
      select: (prompt, options, dialogOptions) =>
        extensionUi.select(prompt, options, dialogOptions),
    ...
    const askQuestion = async (
      ...
      try {
        const { selectedOptions, customInput, navigation, cancelled, timedOut } =
          await askSingleQuestion(ui, q.question, optionLabels, q.multi ?? false, {
    ...
    if (params.questions.length === 1) {
      ...
      const { optionLabels, selectedOptions, customInput, cancelled, timedOut } =
        await askQuestion(q);
...
export const askToolRenderer = {
  renderCall(args: AskRenderArgs, _options: RenderResultOptions, uiTheme: Theme): Component {
    ...
    if (args.questions && args.questions.length > 0) {
      ...
      container.addChild(
        new Text(`${label} ${uiTheme.fg('muted', `${args.questions.length} questions`)}`, 0, 0),
      );
      ...
      for (let i = 0; i < args.questions.length; i++) {
        ...
        container.addChild(
          new Text(
            ` ${uiTheme.fg('dim', qBranch)} ${uiTheme.fg('dim', `[${q.id}]`)}${metaStr}`,
            0,
            0,
          ),
        );
        ...
        if (q.options?.length) {
          ...
          for (let j = 0; j < q.options.length; j++) {
            ...
            const optLabel = renderInlineMarkdown(opt.label, mdTheme, (t) =>
              uiTheme.fg('muted', t),
            );
  ...
  renderResult(
    ...
    if (details.results && details.results.length > 0) {
      ...
      for (let i = 0; i < details.results.length; i++) {
        ...
        container.addChild(
          new Text(
            ` ${uiTheme.fg('dim', branch)} ${statusIcon} ${uiTheme.fg('dim', `[${r.id}]`)}`,
            0,
            0,
          ),
        );
        ...
        if (r.customInput !== undefined) {
          container.addChild(
            new Text(renderCustomInput(uiTheme, continuation, r.customInput, true, false), 0, 0),
          );
    ...
    const header = renderStatusLine(
      { icon: hasSelection ? 'success' : 'warning', title: 'Ask' },
      uiTheme,
    );
    const container = new Container();
    container.addChild(new Text(header, 0, 0));
    ...
    if (details.selectedOptions && details.selectedOptions.length > 0) {
      for (let i = 0; i < details.selectedOptions.length; i++) {
        const isLast =
          i === details.selectedOptions.length - 1 && details.customInput === undefined;
        const branch = isLast ? uiTheme.tree.last : uiTheme.tree.branch;
    ...
    if (details.customInput !== undefined) {
      container.addChild(
        new Text(renderCustomInput(uiTheme, ' ', details.customInput, true, false), 0, 0),
      );
