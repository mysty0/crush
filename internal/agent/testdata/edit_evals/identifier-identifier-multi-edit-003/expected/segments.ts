...
const costSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    const usingSubscription = state.model
      ? ctx.session.modelRegistry.isUsingOAuth(state.model)
      : false;
    ...
    if (normalizedPremiumRequests)
      billingParts.push(`★ ${formatNumber(normalizedPremiumRequests)}`);
...
export const ALL_SEGMENT_IDS: StatusLineSegmentId[] = Object.keys(
  SEGMENTS,
