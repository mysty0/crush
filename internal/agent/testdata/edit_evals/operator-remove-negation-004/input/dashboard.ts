...
function renderExpandedHeader(runtime: AutoresearchRuntime, width: number, theme: Theme): string {
  ...
  const hint = theme.fg(
    'dim',
    ` ctrl+x collapse  ctrl+shift+x overlay${status ? `  ${status}` : ''} `,
  );
  const fillWidth = Math.max(0, width - visibleWidth(label) - visibleWidth(hint));
  return truncateToWidth(
    theme.fg('accent', label) + theme.fg('borderMuted', '-'.repeat(fillWidth)) + hint,
    width,
  );
}
...
function renderCollapsedLine(
  ...
  if (state.confidence !== null) {
    const confidenceColor =
      state.confidence >= 2 ? 'success' : state.confidence >= 1 ? 'warning' : 'error';
    parts.push(theme.fg('dim', ' | '));
  ...
  if (runtime.runningExperiment) {
    parts.push(
      theme.fg(
        'dim',
        ` | running ${formatElapsed(Date.now() - runtime.runningExperiment.startedAt)}`,
      ),
    );
...
export function renderDashboardLines(
  ...
  if (state.results.length > current.length) {
    lines.push(
      truncateToWidth(
        `Archived from earlier segments: ${state.results.length - current.length} runs`,
        width,
      ),
    );
  }
...
function renderTableHeader(state: ExperimentState, width: number, theme: Theme): string {
  const secondaryHeader = state.secondaryMetrics
...
function findBestResult(
  ...
  for (let index = 0; index < state.results.length; index += 1) {
    ...
    if (result.segment !== state.currentSegment || result.status !== 'keep' || result.metric <= 0)
      continue;
