...
export async function readImageMetadata(
  options: ReadImageMetadataOptions,
): Promise<ImageMetadata | null> {
  const resolvedPath = options.resolvedPath ?? resolveReadPath(options.path, options.cwd);
  const mimeType =
    options.detectedMimeType ?? (await detectSupportedImageMimeTypeFromFile(resolvedPath));
...
export async function loadImageInput(
  ...
  let outputMimeType = immeType;
  let outputBytes = inputBuffer.byteLength;
  let dimensionNote: string | undefined;

  if (options.autoResize) {
    try {
      const resized = await resizeImage({ type: 'image', data: outputData, immeType });
