...
function getExecutionDeadlineMs(
  options?: Pick<PythonExecutorOptions, 'deadlineMs' | 'timeoutMs'>,
): number | undefined {
...
async function waitForQueueTurn(
  ...
  if (options.signal?.aborted) {
    throw new PythonExecutionCancelledError(
      isTimedOutCancellation(options.signal.reason, options.signal),
    );
  }
...
async function readPreludeCache(state: PreludeCacheState): Promise<PreludeHelper[] | null> {
  ...
  try {
    ...
    logger.warn('Failed to read Python prelude cache', {
  ...
  try {
    ...
    logger.warn('Failed to parse Python prelude cache', {
      path: state.cachePath,
      error: String(err),
    });
    return null;
...
async function writePreludeCache(
  state: PreludeCacheState,
  helpers: PreludeHelper[],
): Promise<void> {
  ...
  try {
    ...
    logger.warn('Failed to write Python prelude cache', {
      path: state.cachePath,
      error: String(err),
    });
  }
...
async function createKernelSession(
  ...
  try {
    kernel = await logger.timeAsync('createKernelSession:PythonKernel.start', () =>
      PythonKernel.start(startOptions),
    );
  } catch (err) {
...
async function restartKernelSession(
  ...
  try {
    ...
    logger.warn('Failed to shutdown crashed kernel', {
