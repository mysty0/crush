...
function normalizeOptionalNullsForSchema(
  ...
  const required = new Set(
    Array.isArray(schemaObject.required) ? (schemaObject.required as string[]) : [],
  );
...
function coerceArgsFromErrors(
  args: unknown,
  errors:
    | Array<{ keyword?: string; instancePath?: string; params?: { type?: unknown } }>
    | null
    | undefined,
): { value: unknown; changed: boolean } {
