...
function renderType(ty: RustType | null | undefined, depth = 0): string {
  ...
  if ('resolved_path' in ty) {
    const rp = ty.resolved_path as {
      path: string;
      args?: { angle_bracketed?: { args: unknown[] } };
    };
...
function renderGenerics(generics: Generics): string {
  ...
  const params = generics.params
    .filter((p) => p.kind && !('lifetime' in p.kind))
    .map((p) => p.name);
...
function getDocsRsCachePath(target: DocsRsTarget, now = new Date()): string {
  ...
  return path.join(
    getAgentDir(),
    DOCS_RS_CACHE_ROOT,
    `docsrs_${crate}_${version}`,
    DOCS_RS_CACHE_FILENAME,
  );
}
...
function collectInherentMethodLines(
  implIds: number[],
  index: Record<string, RustdocItem>,
): string[] {
...
function collectExplicitTraitImplNames(
  implIds: number[],
  index: Record<string, RustdocItem>,
): string[] {
  const names: string[] = [];
...
function renderModule(
  ...
  for (const id of modData.items) {
    ...
    if (
      item.visibility === 'crate' ||
      (typeof item.visibility === 'object' && 'restricted' in item.visibility)
    )
      continue;
