...
export function renderResult(
  ...
  if (request?.apply !== undefined)
    requestLines.push(theme.fg('dim', `apply: ${request.apply ? 'true' : 'false'}`));
...
function renderDiagnostics(
  ...
  if (expanded) {
    ...
    const items: DiagnosticItem[] =
      parsedDiagnostics.length > 0 ? parsedDiagnostics : fallbackDiagnostics;
  ...
  const remaining =
    (parsedDiagnostics.length > 0 ? parsedDiagnostics.length : fallbackDiagnostics.length) -
    previewItems.length;
