...
func (m *mutableMessageItem) PaintVersion() uint64       { return m.version }

// The item's height tracks m.lines, which changes with m.version, so the
// layout counter moves with it rather than staying pinned.
func (m *mutableMessageItem) LayoutVersion() uint64 { return m.version }
