...
// AgentTasks stubs the sub-agent/background task list. The embedded
// workspace.Workspace is nil, so without this the agent-list layout pass
// (reached via applyPromptQueue -> updateLayoutAndSize) nil-derefs.
func (w *countingWorkspace) AgentTasks(string) []agent.TaskStatus { return nil }
...
func TestSendMessageSetsOptimisticBusy(t *testing.T) {
	...
	// esc right after enter: isAgentBusy gates cancelAgent, so the press
	// must reach it rather than no-op on a stale idle read. The turn has
	// produced no output yet, so this tree cancels immediately instead of
	// arming the two-press confirm (see cancelAgent).
	require.Zero(t, m.promptQueue)
	m.cancelAgent()
	require.False(t, m.isCanceling,
		"nothing generated yet: esc cancels outright rather than arming a confirm")
	require.Equal(t, 1, ws.cancelCalls, "esc must cancel the agent")
}
