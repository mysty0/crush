...
func TestWrite_Success(t *testing.T) {
	...
	require.NoError(t, Write(&b, PageResult{Subject: "linear"}))
	page := b.String()
...
func TestWrite_FailureDoesNotAutoClose(t *testing.T) {
	...
	var b strings.Builder
	require.NoError(t, Write(&b, PageResult{
		Subject:          "sentry",
...
func TestWrite_GrumpyOnFailure(t *testing.T) {
	...
	require.NoError(t, Write(&failed, PageResult{ErrorCode: "access_denied"}))
	require.Contains(t, failed.String(), grumpy)

	var ok strings.Builder
	require.NoError(t, Write(&ok, PageResult{Subject: "linear"}))
...
func TestWrite_TerseFailure(t *testing.T) {
	...
	var b strings.Builder
	require.NoError(t, Write(&b, PageResult{ErrorCode: "server_error"}))
	page := b.String()
...
func TestServe_StatusCodes(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"success": {PageResult{Subject: "linear"}, http.StatusOK},
		"failure": {PageResult{ErrorCode: "access_denied"}, http.StatusBadRequest},
	} {
