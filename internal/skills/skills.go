...
type Skill struct {
	...
	// AutoActivate makes the skill active from the session's first turn
	// without the model calling the skill tool. Users opt out per skill
	// via options.disabled_skills, or per session with "stop <name>".
	AutoActivate  bool              `yaml:"auto-activate" json:"auto_activate"`
